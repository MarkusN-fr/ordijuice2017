#' Perfom CA (Correspondance Analysis) on JUICE data.
#'
#' \code{draw.ca} performs CA (Correspondance Analysis) on the data output
#' from JUICE. It is directly called from the JUICE interface, and one of
#' the centerpieces of ordijuice.
#' JUICE's output is transformed by the function
#' \code{\link{read.check.data}}, that is passed on some of the arguments
#'  listed here, so that \code{draw.ca} can use it.
...
draw.ca <-
...

  {
    ...
    if (newRversion) pb <- myTkProgressBar (paste (ifelse (three.dim, '3D', '2D'),'CA - Analysis progress'), 'Importing data from JUICE', 0, 100, 20) else pb <- NULL
    ...
    last.result <-  use.last (input.data, 'ca', setting = list (nic = 0))

    if (last.result$use.last.result) spec.data.ord <- last.result$last.data.result  else

      spec.data.ord <- cca(input.data$spec.data)
    ...
    #save.ord.result (spec.data.ord, last.result$use.last.result, 'ca', input.data$deleted.plots)
